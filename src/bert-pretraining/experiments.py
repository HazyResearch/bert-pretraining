...
def get_feature_path(exp_name, dataset, ckpt_folder, nbit=32):
    exp_path = "../../results/features/date/exp_name_{}".format(utils.get_date_str())
...
def generate_all_sentiment_features_dimensionality(ckpt_folders):
            ...
            for dataset in datasets:
                for nbit in nbits:
                    output_path = get_feature_path(exp_name, 
                        dataset=dataset, ckpt_folder=ckpt_folder, nbit=nbit)
                    for part in ['train', 'test', 'heldout']:
                        input_file = data_path + "/{}.{}.txt".format(dataset, part)
                        output_file = output_path + "/{}.{}.feature.npy"
