...
def bert_pretraining_lr_tuning_evaluation():
    ...
    run_tmp = ('python ./third_party/bert/run_pretraining.py \
            ...
            --bert_config_file=../../data/bert/3_layer_bert_config.json \
            --eval_batch_size=256 \
            --init_ckptpoint=gs://embeddings-ckpt/bert_pretraining_lr_tuning/pretrain_tuning_lr_{}/model.ckpt-250000 \
            --max_seq_length=128 \
            --max_predictions_per_seq=20 \
            --use_tpu=True \
            --tpu_name=tpu-{} 2>&1 | tee output/pretrain_tuning_lr_{}_eval.log &')
    with open(file_name, 'w') as f:
        for i, lr in enumerate(lrs):
           cmd_str = tpu_tmp.format(i, i)
           f.write(cmd_str + "\n")
